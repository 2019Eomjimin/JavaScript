...
// 객체 생성 시 객체 밖에서 점 포기법으로 프로퍼티 이름과 값 지정 가능
let student = {
    name : "엄지민"
}
student.age = 24;
...
// 이때 생성자 함수로 객체를 정의하여 사용 가능
function makeUser(name, age) {
    return {
        name : name,
        age : age   // 변수와 프로퍼티 이름이 같음
    }
}


let user1 = makeUser("엄지민", 24)
console.log("user1 : ", user1)

// 위 코드에서 객체 정의할때 변수와 프로퍼티 이름이 같으면
// 줄여서 사용 가능
function makeUser(name, age) {
    return {
        name : name,
...
// 심벌키에 접근
// [] (대괄호)를 사용하여 프롶퍼티나 메서드에 접근
// 메서드 []()
console.log("member[id] : ", member[id]);
//console.log(member[tel]());

// 심벌은 만들때 마다 유일한 값을 가짐
// 상황에 따라 만든 심벌을 다른 곳에서 사용해야 할 경우가 있음
// 여러 곳에서 같이 사용할 수 있는 심벌을 전역 심벌이라함
// Symbol.for() 메서드
let tel_num = Symbol.for("tle_num");
let phone = Symbol.for("tle_num");
